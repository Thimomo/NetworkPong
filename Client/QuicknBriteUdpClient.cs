...
class QuicknBriteUdpClient
{
    ...
    BaseProject.Game1 mainGame;

    IPEndPoint serverEndpoint;

    public void StartConnection(string ip, int sendPort, int receivePort, BaseProject.Game1 mn)
    {
        this.ip = ip;
        sendingPort = sendPort;
        receivingPort = receivePort;
        mainGame = mn; //Used for handling messages
    ...
    private void ListenForMessages(UdpClient client)
    {
        ...
        while (threadRunning)
        {
            try
            {
                ...
                mainGame.HandleMessage(receivingPort, receiveBytes);
